<script lang="ts">
  ...
  import { Icon } from '$lib/modules/client/common';
...
<div class="app">
  ...
  <header class="header">
    <div class="header-content">
      ...
      <div class="nav-right">
        <Pill text={statusLabels[systemStatus] || 'Checking'} classes={statusClasses[systemStatus] || 'pill-status-unknown'} />
        <Button
          classes="btn-gear {$page.url.pathname === '/config' ? 'btn-gear-active' : ''}"
          onclick={() => goto('/config')}
          ariaLabel="Settings"
        >
          {#snippet icon()}<Icon name="settings" size={18} />{/snippet}
        </Button>
      </div>
    </div>
  </header>
